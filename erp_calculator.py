...
def validate_against_damodaran():
    ...
    print()
    print("──────────────────────────────────────────────────────")
    print("  Reconciliation vs Damodaran histimpl.xls (Jan 2026 update)")
    print("──────────────────────────────────────────────────────")
    print("  Damodaran's published Implied ERP, year-end 2025:    4.2300%")
    print(f"  Our FCFE solver, hardcoded inputs above:             {result.implied_erp:.4%}")
    print(f"  Gap (this point):                                    {(result.implied_erp - 0.0423) * 1e4:+.0f} bp")
    print()
    print("  Phase 6 Track A cross-time check (scripts/reconcile_damodaran.py):")
    print("    The hardcoded S&P 5881.63 above is early-Jan 2026 spot.")
    print("    Damodaran used the year-end 2025 close, S&P=6845.50.")
    print("    With the corrected S&P, our FCFE solver returns 4.27% —")
    print("    within ~4bp of his 4.23%. ⇒ The 73bp 'gap' is input")
    print("    transcription, not a solver-methodology defect.")
    print("    Cross-time DDM reproduction of his published series shows")
    print("    a -32bp median delta (2011-2025) — that's the expected")
    print("    DDM-vs-FCFE methodology shift Damodaran made ~2014.")
    print()
    print("  Solver math is unchanged in Phase 6 Track A by design;")
    print("  see SHARED_NOTES 2026-05-13 entry for the full investigation.")
    print("──────────────────────────────────────────────────────")
